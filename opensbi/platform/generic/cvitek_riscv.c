...
/* RTOS Memory Protection Configuration */
#define RTOS_BASE_ADDR   0x8FE00000UL
#define RTOS_SIZE        0x200000UL   /* 2MB */
...
{
	int rc;
	struct sbi_domain_memregion rtos_region;

	sbi_system_suspend_set_device(&cvitek_sbi_suspend_device);

	/* Only configure PMP once during cold boot */
	if (!cold_boot)
		return 0;

	/*
	 * Add RTOS protection region to the root domain.
	 * Flags = 0 means: No R, No W, No X for S-Mode.
	 * This effectively DENIES S-Mode access to the RTOS region.
	 */
	sbi_domain_memregion_init(RTOS_BASE_ADDR, RTOS_SIZE, 0, &rtos_region);
	rc = sbi_domain_root_add_memregion(&rtos_region);
	if (rc) {
		sbi_printf("CVITEK: Failed to add RTOS PMP region (err=%d)\n", rc);
		return rc;
	}

	sbi_printf("CVITEK: RTOS PMP protection enabled [0x%lx - 0x%lx]\n",
		   RTOS_BASE_ADDR, RTOS_BASE_ADDR + RTOS_SIZE - 1);
